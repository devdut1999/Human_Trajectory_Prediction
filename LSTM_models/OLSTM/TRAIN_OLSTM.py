...
from OLSTM_model import OLSTM
import pandas as pd
import numpy as np
import OLSTM_utils as utils
import os
import random
from tensorboardX import SummaryWriter
from Data_loader_OLSTM import DataLoader
import math
import argparse
...
def save_imgs(batch, img_path, idx, n_neighbors=5, xlim=[-150, 150], ylim=[-150, 150]):
    ...
    results, pts = net.forward(
        batch_for_network[:obs, :, :], grids, batch[2], first_pos)

    idx__ = idx*batch_size
    '''
...
def test_function(model, test_data, save_plot, epoch, it, save_dir=('imgs_vlstm_nr/', 'checkpoints_vlstm_nr/'), num=12, comment='', plot_loss=False):
    ...
    saved_for_plots = []
    obs = 8
    loss_display_test, rmse_display, l2_display, final_l2_display, avg_collision_predicted, avg_collision_true = 0, 0, 0, 0, 0, 0
    ...
    for idx, batch in enumerate(test_data):
        ...
        acc_l2, acc_final = utils.get_accuracy(
            tmp_true_pos[obs:, :, :][:, :, [2, 3]], pts_predicted)
        avg_collision_predicted += utils.compute_avg_collision(
            pts_predicted, batch[2], 8)
        avg_collision_true += utils.compute_avg_collision(
            batch_for_network[8:, :, :][:, :, [2, 3]].data, batch[2], 8)
        ...
        if (idx in saved_plots) and save_plot:
            ...
            utils.plot_trajectory(batch[0][x].squeeze(0), plot_pred[:, x, :], batch[2][
                                  x], 5, "{}o_lstm_it_{}_i_{}_b_{}_test_{}".format(save_dir[0], epoch, idx, x, comment))

    if save_plot:
        utils.save_checkpoint({
            'true_pred': saved_for_plots,
        }, save_dir[1] + 'test_data_plot_o_lstm_{}.pth.tar'.format(comment))
    ...
    print('[Avg_collision_Predicted_Test_{}] {}'.format(comment,
                                                        avg_collision_predicted / len(test_data)))
    print('[Avg_collision_True_Test_{}] {}'.format(comment,
                                                   avg_collision_true / len(test_data)))
    if (plot_loss) and (writer is not None):
        ...
        writer.add_scalar('Avg_collision_test {}'.format(
            comment), avg_collision_predicted / len(test_data), it)
    print('-------------------------------------------------------------')
...
parser.add_argument('--plot_save', type=str, default='olstm',
...
save_dir = ('CKPTS/olstm_imgs_{}/'.format(args_input.plot_save),
            'IMGS/olstm_checkpoints_{}/'.format(args_input.plot_save))
...
args = {'embedded_input': embedded_input, 'hidden_size': hidden_size,
        'grid_size': args_input.grid_size, 'max_dist': max_dist_scaled, 'embedding_occupancy_map': args_input.embedding_occupancy_map_size, 'use_speeds': use_speeds}
net = OLSTM(args)
...
if args_input.eval_folder is None:
    ...
    for epoch in range(min_epoch, nb_iter):
        ...
        loss_display, rmse_display, l2_display, final_l2_display, avg_collision_predicted, avg_collision_true = 0, 0, 0, 0, 0, 0
        ...
        print('[avg_collision_predicted] {}'.format(
            avg_collision_predicted / len(train_data)))
        print('[avg_collision_true] {}'.format(
            avg_collision_true / len(train_data)))
        print('-------------------------------------------------------------')
        ...
        if epoch % 4 == 0:
            utils.save_checkpoint({
                ...
                'iteration': it + 1,
                'epoch': epoch + 1,
            }, save_dir[1] + 'checkpoint_o_lstm_{}.pth.tar'.format(epoch))
            utils.save_checkpoint({
                'true_pred': saved_for_plots,
            }, save_dir[1] + 'data_plot_o_lstm.pth.tar')
            print('Model saved')
            print()
    writer.close()
